...
require('dotenv').config(); // 👈 THIS IS THE CRITICAL MISSING LINE

// We initialize the client inside a constant to check if it worked
const apiKey = process.env.GROQ_API_KEY;

// Safe initialization to prevent the crash you saw in your terminal
const groq = apiKey ? new Groq({ apiKey }) : null;

async function generateAnswer({ patientName, disease, query, location, publications, trials }) {
  try {
    // If the key is missing (like on your local machine), don't crash, just return the error
    if (!groq) {
    ...
    if (!publications || publications.length === 0) {
      return "CuraLink retrieved data but needs more publications for a full synthesis.";
    }
